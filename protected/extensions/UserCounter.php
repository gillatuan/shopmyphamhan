...
// Übergabeparameter für den Aufruf des Counters
    ...
    {
        $cfg = $_SERVER['REMOTE_ADDR'] != '127.0.0.1' ? require dirname(__FILE__) . '/../config/main.php' : require dirname(__FILE__) . '/../config/dev.php';
        //print '<pre>';
    ...
    {
        $base_url = Helper::basePath() . "/../images/";
        $default_style = 'blue';
        ...
        $len = strlen($value);
        for ($i = 0; $i < $len; $i++) {
            if (!file_exists($base_url . $style_dir . substr($value, $i, 1) . '.' . $ext)) {
                mkdir($base_url . $style_dir . substr($value, $i, 1) . '.' . $ext);
            }
